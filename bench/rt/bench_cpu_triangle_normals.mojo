...
def trace_recomputed_normal(
    ...
    var base = tri_idx * 3
    ref v0 = world.scene.triangle_vertices[base + 0]
    ref v1 = world.scene.triangle_vertices[base + 1]
    ref v2 = world.scene.triangle_vertices[base + 2]
    ...
    return HitRecord(
        ...
        p,
        normal,
        world.scene.triangle_surfaces[tri_idx].copy(),
        bvh_hit.t,
        front_face,
    )
...
](world: World[], rays: List[Rayf32[Frame.WORLD]]) -> Tuple[Float64, Int]:
    var checksum = 0.0
    var hits = 0
    for ray in rays:
...
def time_rays[
    reuse_bvh_normal: Bool
](world: World[], rays: List[Rayf32[Frame.WORLD]]) -> TimingResult:
