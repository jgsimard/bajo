...
struct Matrix4(Stringable):
    var data : InlineArray[Float32, 16]

    fn __init__(out self, data: InlineArray[Float32, 16]):
        self.data = data

    fn __init__(out self, uninitialized: Bool):
        self.data = InlineArray[Float32, 16](uninitialized=True)
    ...
    fn inverse_simd(self) raises -> Matrix4:
        """
...
fn bench_inv[use_simd: Bool]() raises:
    ...
    var mat = Matrix4(matrix_data)
    var mat_inv = Matrix4(uninitialized=True)
    for _ in range(1e6):
        @parameter
        if use_simd:
            mat_inv = mat.inverse_simd()
        else:
            mat_inv = mat.inverse()
...
fn main() raises:
    ...
    var mat_inv = mat.inverse()
    print(String(mat_inv))

    var mat_inv_simd = mat.inverse_simd()
