...
from bajo.core import Affine3f32, Vec3f32, assert_vec_equal, dot, length
...
def test_world_hit_maps_material_and_normal() raises:
    ...
    add_sphere(spheres, sphere_surfaces, Vec3f32(0.0, 0.0, -1.0), 0.5, matte)
    var world = World(
        spheres^,
        sphere_surfaces^,
        triangle_vertices^,
...
def test_world_preserves_signed_radius_normals() raises:
    ...
    add_sphere(spheres, sphere_surfaces, Vec3f32(0.0, 0.0, -1.0), -0.5, glass)
    var world = World(
        spheres^,
        sphere_surfaces^,
        triangle_vertices^,
...
def test_triangle_mesh_instances_use_instance_surfaces() raises:
    ...
    var hit1 = (
        world.hit(Ray(Vec3f32(1.5, 0.0, 0.0), Vec3f32(0.0, 0.0, -1.0)))
        .value()
        .copy()
    )
